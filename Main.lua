...
LimbTab:CreateSlider({
   Name = "Transparency Limb",
   Range = {0, 1},
   Increment = 0.1,
   CurrentValue = 0.9,
   Callback = function(Value) Settings.Transparency = Value end,
})
...
LimbTab:CreateToggle({
   ...
   CurrentValue = false,
   Callback = function(Value) Settings.Collision = Value end,
})
...
task.spawn(function()
    while task.wait(0.5) do
        if Settings.Enabled then
                ...
                if obj:IsA("Model") then
                    ...
                    if hum and not isPlayer then
                        ...
                        if Settings.TeamCheck then
                            local lp = game.Players.LocalPlayer
                            if (obj:FindFirstChild("TeamColor") and obj.TeamColor == lp.TeamColor) then
                                isEnemy = false
                            end
                        end

                        if not isShop and isEnemy then
                            local target = obj:FindFirstChild(Settings.SelectedPart) or obj:FindFirstChild("HumanoidRootPart")
                            ...
                            if target and target:IsA("BasePart") then
                                ...
                                target.CanCollide = Settings.Collision
            ...
            for part in pairs(OldSizes) do
                if not part or not part.Parent then OldSizes[part] = nil end
            end
        end
    end
end)
